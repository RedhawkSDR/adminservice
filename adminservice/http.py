...
from adminservice.medusa import http_date
from adminservice.medusa import http_server
from adminservice.medusa import producers
from adminservice.medusa import filesys
from adminservice.medusa import default_handler

from adminservice.medusa.auth_handler import auth_handler
...
class adminservice_http_server(http_server.http_server):
    ...
    def prebind(self, sock, logger_object):
        ...
        from adminservice.medusa import logger
    ...
    def postbind(self):
        from adminservice.medusa.counter import counter
        from adminservice.medusa.http_server import VERSION_STRING
...
class adminservice_af_inet_http_server(adminservice_http_server):
    """ AF_INET version of adminservice HTTP server """
...
class adminservice_af_unix_http_server(adminservice_http_server):
    """ AF_UNIX version of adminservice HTTP server """
        ...
        while 1:
            ...
            try:
                ...
                try:
                    ...
                    if used:
                        # cooperate with 'openhttpserver' in adminserviced
...
def make_http_servers(options, adminserviced):
    servers = []
    ...
    for config in options.server_configs:
        ...
        xmlrpchandler = adminservice_xmlrpc_handler(adminserviced, subinterfaces)
        tailhandler = logtail_handler(adminserviced)
        maintailhandler = mainlogtail_handler(adminserviced)
        uihandler = adminservice_ui_handler(adminserviced)
        ...
        if username:
            ...
            xmlrpchandler = adminservice_auth_handler(users, xmlrpchandler)
            tailhandler = adminservice_auth_handler(users, tailhandler)
            maintailhandler = adminservice_auth_handler(users, maintailhandler)
            uihandler = adminservice_auth_handler(users, uihandler)
            defaulthandler = adminservice_auth_handler(users, defaulthandler)
        else:
...
class LogWrapper:
    ...
    them to the adminservice logger'''
...
class adminservice_auth_handler(auth_handler):
