...
from adminservice.tests.base import DummyPGroupConfig
from adminservice.tests.base import DummyDispatcher
from adminservice.tests.base import DummyEvent
from adminservice.tests.base import DummyFCGIGroupConfig
from adminservice.tests.base import DummySocketConfig
from adminservice.tests.base import DummyProcessGroup
from adminservice.tests.base import DummyFCGIProcessGroup

from adminservice.process import Subprocess
from adminservice.options import BadCommand
...
class SubprocessTests(unittest.TestCase):
    ...
    def tearDown(self):
        from adminservice.events import clear
        clear()

    def test_getProcessStateDescription(self):
        from adminservice.states import ProcessStates
        from adminservice.process import getProcessStateDescription
    ...
    def test_spawn_as_child_setuid_ok(self):
        ...
        self.assertEqual(options.written,
             {2: "adminservice: child process was not spawned\n"})
    ...
    def test_spawn_as_child_setuid_fail(self):
        ...
        self.assertEqual(len(options.fds_closed), options.minfds - 3)
        self.assertEqual(options.written,
             {2: "adminservice: couldn't setuid to 1: failure reason\n"
                 "adminservice: child process was not spawned\n"})
    ...
    def test_spawn_as_child_cwd_fail(self):
        ...
        out = {2: "adminservice: couldn't chdir to /tmp: ENOENT\n"
                  "adminservice: child process was not spawned\n"}
    ...
    def test_spawn_as_child_execv_fail_oserror(self):
        ...
        self.assertEqual(len(options.duped), 3)
        self.assertEqual(len(options.fds_closed), options.minfds - 3)
        out = {2: "adminservice: couldn't exec /good/filename: EPERM\n"
                  "adminservice: child process was not spawned\n"}
        self.assertEqual(options.written, out)
        self.assertEqual(options.privsdropped, None)
        self.assertEqual(options._exitcode, 127)
    ...
    def test_spawn_as_child_execv_fail_runtime_error(self):
        ...
        head = "adminservice: couldn't exec /good/filename:"
    ...
    def _dont_test_spawn_and_kill(self):
        ...
        from adminservice.tests.base import makeSpew
        try:
            sigchlds = []
            def sighandler(*args):
    ...
    def test_transition_stopped_to_starting_supervisor_stopping(self):
        ...
        L = []
        events.subscribe(events.ProcessStateEvent, lambda x: L.append(x))
        from adminservice.states import ProcessStates, SupervisorStates
        options = DummyOptions()
        options.mood = SupervisorStates.SHUTDOWN

        # this should not be spawned, as adminservice is shutting down
    ...
    def test_transition_exited_to_starting_condit_supervisor_running(self):
        ...
        pconfig = DummyPConfig(options, 'process', 'process','/bin/process')
        from adminservice.datatypes import RestartWhenExitUnexpected
...
class ProcessGroupBaseTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.process import ProcessGroupBase
...
class ProcessGroupTests(ProcessGroupBaseTests):
    ...
    def test_repr(self):
        ...
        self.assertTrue(s.startswith(
            '<adminservice.process.ProcessGroup instance at'), s)
...
class EventListenerPoolTests(ProcessGroupBaseTests):
    ...
    def test__acceptEvent_attaches_pool_serial_and_serial(self):
        from adminservice.process import GlobalSerial
        options = DummyOptions()
        gconfig = DummyPGroupConfig(options)
        pconfig1 = DummyPConfig(options, 'process1', 'process1','/bin/process1')
        gconfig = DummyPGroupConfig(options, pconfigs=[pconfig1])
    ...
    def test_repr(self):
        ...
        s = repr(pool)
        self.assertTrue(s.startswith(
            '<adminservice.process.EventListenerPool instance at'))
...
class test_new_serial(unittest.TestCase):
    def _callFUT(self, inst):
        from adminservice.process import new_serial
