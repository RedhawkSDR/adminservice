"""Test suite for adminservice.options"""
...
from adminservice.loggers import LevelsByName

from adminservice.tests.base import DummySupervisor
from adminservice.tests.base import DummyLogger
from adminservice.tests.base import DummyOptions
from adminservice.tests.base import DummyPoller
from adminservice.tests.base import DummyPConfig
from adminservice.tests.base import DummyProcess
from adminservice.tests.base import DummySocketConfig
from adminservice.tests.base import lstrip

class OptionTests(unittest.TestCase):

    def _getTargetClass(self):
        from adminservice.options import Options
        return Options
    ...
    def test_searchpaths(self):
        ...
        self.assertEqual(options.searchpaths[-4:], [
            ...
            '/etc/supervisord.conf',
            '/etc/adminservice/supervisord.conf',
            ])
...
class ServerOptionsTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.options import ServerOptions
    ...
    def test_options(self):
        ...
        from adminservice import datatypes
    ...
    def test_options_afunix(self):
        ...
        ; ...
        """)
        from adminservice.options import UnhosedConfigParser
    ...
    def test_event_listener_pools_from_parser(self):
        ...
        """)
        from adminservice.options import UnhosedConfigParser
        from adminservice.dispatchers import default_handler
        config = UnhosedConfigParser()
        config.read_string(text)
        instance = self._makeOne()
        gconfigs = instance.process_groups_from_parser(config)
    ...
    def test_event_listener_pool_with_event_result_handler(self):
        ...
        events=PROCESS_COMMUNICATION
        command = /bin/dog
        result_handler = adminservice.tests.base:dummy_handler
        """)
        from adminservice.options import UnhosedConfigParser
        from adminservice.tests.base import dummy_handler
        config = UnhosedConfigParser()
        config.read_string(text)
        instance = self._makeOne()
        gconfigs = instance.process_groups_from_parser(config)
    ...
    def test_event_listener_pool_result_handler_unimportable(self):
        ...
        events=PROCESS_COMMUNICATION
        command = /bin/cat
        result_handler = adminservice.tests.base:nonexistant
        """)
        from adminservice.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        ...
        try:
            ...
            self.assertEqual(exc.args[0],
                'adminservice.tests.base:nonexistant cannot be '
    ...
    def test_fcgi_programs_from_parser(self):
        from adminservice.options import FastCGIGroupConfig
        from adminservice.options import FastCGIProcessConfig
        text = lstrip("""\
        [fcgi-program:foo]
    ...
    def test_rpcinterfaces_from_parser(self):
        text = lstrip("""\
        [rpcinterface:dummy]
        adminservice.rpcinterface_factory = %s
        foo = bar
        baz = qux
        """ % __name__)
        from adminservice.options import UnhosedConfigParser
    ...
    def test_rpcinterfaces_from_parser_factory_expansions(self):
        text = lstrip("""\
        [rpcinterface:dummy]
        adminservice.rpcinterface_factory = %(factory)s
        foo = %(pet)s
        """)
        from adminservice.options import UnhosedConfigParser
        instance = self._makeOne()
    ...
    def test_rpcinterfaces_from_parser_factory_missing(self):
        text = lstrip("""\
        [rpcinterface:dummy]
        # note: no adminservice.rpcinterface_factory here
        """)
        from adminservice.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        ...
        try:
            ...
            self.assertEqual(exc.args[0], 'section [rpcinterface:dummy] '
                'does not specify a adminservice.rpcinterface_factory')

    def test_rpcinterfaces_from_parser_factory_not_importable(self):
        text = lstrip("""\
        [rpcinterface:dummy]
        adminservice.rpcinterface_factory = nonexistant
        """)
        from adminservice.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
    ...
    def test_openhttpservers_does_not_catch_other_exception_types(self):
        ...
        # this scenario probably means a bug in adminservice.  we dump
...
class TestProcessConfig(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.options import ProcessConfig
    ...
    def test_make_dispatchers_stderr_not_redirected(self):
        ...
        from adminservice.events import ProcessCommunicationStdoutEvent
        ...
        from adminservice.events import ProcessCommunicationStderrEvent
...
class EventListenerConfigTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.options import EventListenerConfig
    ...
    def test_make_dispatchers(self):
        ...
        try:
            ...
            self.assertEqual(dispatchers[5].channel, 'stdout')
            from adminservice.states import EventListenerStates
...
class ProcessGroupConfigTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.options import ProcessGroupConfig
...
class EventListenerPoolConfigTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.options import EventListenerPoolConfig
...
class SignalReceiverTests(unittest.TestCase):
    def test_returns_None_initially(self):
        from adminservice.options import SignalReceiver
        sr = SignalReceiver()
