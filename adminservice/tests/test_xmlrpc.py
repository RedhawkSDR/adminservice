...
from adminservice.tests.base import DummyRequest
from adminservice.tests.base import DummySupervisorRPCNamespace
...
class XMLRPCHandlerTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.xmlrpc import supervisor_xmlrpc_handler
    ...
    def test_ctor(self):
        ...
        subinterfaces = [('adminservice', DummySupervisorRPCNamespace())]
    ...
    def test_continue_request_nosuchmethod(self):
        ...
        self.assertEqual(logdata[-2],
                         u'XML-RPC method called: adminservice.noSuchMethod()')
        self.assertEqual(logdata[-1],
           (u'XML-RPC method adminservice.noSuchMethod() returned fault: '
            '[1] UNKNOWN_METHOD'))
    ...
    def test_continue_request_methodsuccess(self):
        ...
        handler = self._makeOne(supervisor, subinterfaces)
        import xmlrpclib
        data = xmlrpclib.dumps((), 'adminservice.getAPIVersion')
        request = DummyRequest('/what/ever', None, None, None)
        handler.continue_request(data, request)
        logdata = supervisor.options.logger.data
        from adminservice.xmlrpc import loads
        ...
        self.assertEqual(len(logdata), expected)
        self.assertEqual(logdata[-2],
               u'XML-RPC method called: adminservice.getAPIVersion()')
        self.assertEqual(logdata[-1],
            u'XML-RPC method adminservice.getAPIVersion() returned successfully')
        self.assertEqual(len(request.producers), 1)
        xml_response = request.producers[0]
        response = xmlrpclib.loads(xml_response)
        from adminservice.rpcinterface import API_VERSION
    ...
    def test_continue_request_no_params_in_request(self):
        ...
        data = '<?xml version="1.0" encoding="UTF-8"?>' \
               '<methodCall>' \
               '<methodName>adminservice.getAPIVersion</methodName>' \
               '</methodCall>'
        request = DummyRequest('/what/ever', None, None, None)
        handler.continue_request(data, request)
        logdata = supervisor.options.logger.data
    ...
    def test_continue_request_500_if_rpcinterface_method_call_raises(self):
        ...
        handler = self._makeOne(supervisor, subinterfaces)
        import xmlrpclib
        data = xmlrpclib.dumps((), 'adminservice.raiseError')
        request = DummyRequest('/what/ever', None, None, None)
        handler.continue_request(data, request)
        logdata = supervisor.options.logger.data
        from adminservice.xmlrpc import loads
        ...
        self.assertEqual(len(logdata), expected)
        self.assertEqual(logdata[-2],
               u'XML-RPC method called: adminservice.raiseError()')
    ...
    def test_continue_request_500_if_xmlrpc_dumps_raises(self):
        ...
        handler = self._makeOne(supervisor, subinterfaces)
        import xmlrpclib
        data = xmlrpclib.dumps((), 'adminservice.getXmlRpcUnmarshallable')
        request = DummyRequest('/what/ever', None, None, None)
        handler.continue_request(data, request)
        logdata = supervisor.options.logger.data
        from adminservice.xmlrpc import loads
        ...
        self.assertEqual(logdata[-3],
               'XML-RPC method called: adminservice.getXmlRpcUnmarshallable()')
        self.assertEqual(logdata[-2],
               'XML-RPC method adminservice.getXmlRpcUnmarshallable() '
...
class SupervisorTransportTests(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.xmlrpc import SupervisorTransport
...
class TestDeferredXMLRPCResponse(unittest.TestCase):
    def _getTargetClass(self):
        from adminservice.xmlrpc import DeferredXMLRPCResponse
    ...
    def test_more_callback_raises_RPCError(self):
        from adminservice.xmlrpc import RPCError, Faults
        def callback():
...
class Test_gettags(unittest.TestCase):
    def _callFUT(self, comment):
        from adminservice.xmlrpc import gettags
...
class Test_capped_int(unittest.TestCase):
    def _callFUT(self, value):
        from adminservice.xmlrpc import capped_int
