...
class POutputDispatcherTests(unittest.TestCase):
    ...
    def test_stdout_capturemode_single_buffer(self):
        ...
        from adminservice.events import ProcessCommunicationEvent
        from adminservice.events import subscribe
        events = []
        def doit(event):
            events.append(event)
        ...
        options = DummyOptions()
        from adminservice.loggers import getLogger
    ...
    def test_repr(self):
        ...
        self.assertNotEqual(
            drepr.find('<adminservice.tests.base.DummyProcess instance at'),
            -1)
...
class PEventListenerDispatcherTests(unittest.TestCase):
    ...
    def test_handle_listener_state_change_busy_to_unknown(self):
        from adminservice.events import EventRejectedEvent
        from adminservice.events import subscribe
        events = []
        def doit(event):
            events.append(event)
    ...
    def test_handle_result_rejectevent(self):
        ...
        def rejected(event, result):
            from adminservice.dispatchers import RejectEvent
...
class stripEscapeTests(unittest.TestCase):
    def _callFUT(self, s):
        from adminservice.dispatchers import stripEscapes
