...
"""adminserviced -- run a set of applications as daemons.
...
-v/--version -- print adminserviced version number and exit
-u/--user USER -- run adminserviced as this user (or numeric uid)
-g/--group GROUP -- run adminserviced as this group (or numeric uid)
...
-i/--identifier STR -- identifier used for this instance of adminserviced
...
--profile_options OPTIONS -- run adminserviced under profiler and output
    ...
    def handle_signal(self):
        ...
        if sig:
            if sig in (signal.SIGTERM, signal.SIGINT, signal.SIGQUIT):
                ...
                self.options.mood = AdminServiceStates.SHUTDOWN
            elif sig == signal.SIGHUP:
                self.options.logger.warn(
                    'received %s indicating restart request' % signame(sig))
                self.options.mood = AdminServiceStates.RESTARTING
...
def main(args=None, test=False):
    ...
    # if we hup, restart by making a new AdminService()
    first = True
    while 1:
        ...
        first = False
        if test or (options.mood < AdminServiceStates.RESTARTING):
            break

def go(options):
    d = AdminService(options)
    try:
        d.main()
    except asyncore.ExitNow:
        pass
