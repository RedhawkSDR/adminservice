...
from adminservice.datatypes import process_or_group_name
from adminservice.datatypes import boolean
from adminservice.datatypes import integer
from adminservice.datatypes import name_to_gid
from adminservice.datatypes import name_to_uid
from adminservice.datatypes import gid_for_uid
from adminservice.datatypes import existing_dirpath
from adminservice.datatypes import byte_size
from adminservice.datatypes import signal_number
from adminservice.datatypes import list_of_exitcodes
from adminservice.datatypes import dict_of_key_value_pairs
from adminservice.datatypes import logfile_name
from adminservice.datatypes import list_of_strings
from adminservice.datatypes import octal_type
from adminservice.datatypes import existing_directory
from adminservice.datatypes import logging_level
from adminservice.datatypes import colon_separated_user_group
from adminservice.datatypes import inet_address
from adminservice.datatypes import InetStreamSocketConfig
from adminservice.datatypes import UnixStreamSocketConfig
from adminservice.datatypes import url
from adminservice.datatypes import Automatic
from adminservice.datatypes import debug_level
from adminservice.datatypes import auto_restart
from adminservice.datatypes import profile_options

from adminservice import loggers
from adminservice import states
from adminservice import xmlrpc
from adminservice import poller
...
class ServerOptions(Options):
    ...
    mood = states.AdminServiceStates.RUNNING

    def __init__(self):
        Options.__init__(self)
        self.configroot = Dummy()
        self.configroot.adminserviced = Dummy()

        self.add(None, None, "v", "version", self.version)
        self.add("nodaemon", "adminserviced.nodaemon", "n", "nodaemon", flag=1,
                 default=0)
        self.add("user", "adminserviced.user", "u:", "user=")
        self.add("group", "adminserviced.group", "g:", "group=")
        self.add("umask", "adminserviced.umask", "m:", "umask=",
                 octal_type, default='022')
        self.add("directory", "adminserviced.directory", "d:", "directory=",
                 existing_directory, default='/var/redhawk/sdr')
        self.add("logfile", "adminserviced.logfile", "l:", "logfile=",
                 existing_dirpath, default="adminserviced.log")
        self.add("logfile_maxbytes", "adminserviced.logfile_maxbytes",
                 "y:", "logfile_maxbytes=", byte_size,
                 default=50 * 1024 * 1024) # 50MB
        self.add("logfile_backups", "adminserviced.logfile_backups",
                 "z:", "logfile_backups=", integer, default=10)
        self.add("loglevel", "adminserviced.loglevel", "e:", "loglevel=",
                 logging_level, default="info")
        self.add("pidfile", "adminserviced.pidfile", "j:", "pidfile=",
        ...
        self.add("minfds", "adminserviced.minfds",
                 "a:", "minfds=", int, default=1024)
        self.add("minprocs", "adminserviced.minprocs",
                 "", "minprocs=", int, default=200)
        self.add("nocleanup", "adminserviced.nocleanup",
                 "k", "nocleanup", flag=1, default=0)
        self.add("strip_ansi", "adminserviced.strip_ansi",
                 "t", "strip_ansi", flag=1, default=0)
        self.add("profile_options", "adminserviced.profile_options",
    ...
    def realize(self, *arg, **kw):
        ...
        section = self.configroot.adminserviced
    ...
    def process_rh_defaults(self):
        ...
        configDir = os.path.join(self.configroot.adminserviced.config_dir,
                                 self.configroot.adminserviced.domains_dir)
        ...
        configDir = os.path.join(self.configroot.adminserviced.config_dir,
                                 self.configroot.adminserviced.nodes_dir)
        if os.path.exists(defaultsFile):
            parser = self.parse_rh_config(defaultsFile, configDir)
        ...
        configDir = os.path.join(self.configroot.adminserviced.config_dir,
                                 self.configroot.adminserviced.waveforms_dir)
        if os.path.exists(defaultsFile):
            parser = self.parse_rh_config(defaultsFile, configDir)
    ...
    def read_config(self, fp):
        ...
        sections = parser.sections()
        if not 'adminserviced' in sections:
            raise ValueError('.ini file does not include adminserviced section')
        ...
        section.logfile = existing_dirpath(get('logfile', 'adminserviced.log'))
        ...
        section.pidfile = existing_dirpath(get('pidfile', 'adminserviced.pid'))
        ...
        section.rpcinterface_factories = self.get_plugins(
            parser,
            'adminservice.rpcinterface_factory',
            'rpcinterface:'
            )
    ...
    def process_groups_from_parser(self, parser, config_class=None):
        ...
        for section in all_sections:
            ...
            result_handler = get(section, 'result_handler',
                                       'adminservice.dispatchers:default_handler')
            try:
            ...
            from adminservice.events import EventTypes
            pool_events = []
    ...
    def _daemonize(self):
        ...
        if pid != 0:
            # Parent
            self.logger.blather("adminserviced forked; parent exiting")
            os._exit(0)
        # Child
        self.logger.info("daemonizing the adminserviced process")
        if self.directory:
            try:
    ...
    def write_pidfile(self):
        ...
        try:
            ...
            self.logger.info('adminserviced started with pid %s' % pid)
    ...
    def openhttpservers(self, adminserviced):
        try:
            self.httpservers = self.make_http_servers(adminserviced)
            ...
            if why.args[0] == errno.EADDRINUSE:
                self.usage('Another program is already listening on '
                           ...
                           'down first before starting adminserviced.')
            else:
    ...
    def drop_privileges(self, user, group):
        ...
        username or uid.  Called for adminserviced startup and when spawning
        ...
        if current_uid == uid:
            ...
            # current one.  this allows a adminserviced running as an
    ...
    def set_uid_or_exit(self):
        """Set the uid of the adminserviced process.  Called during adminserviced
        startup only.  No return value.  Exits the process via usage() if
        privileges could not be dropped."""
        if self.uid is None:
            if os.getuid() == 0:
                self.parse_criticals.append('AdminService is running as root.  '
    ...
    def set_rlimits_or_exit(self):
        """Set the rlimits of the adminserviced process.  Called during
        adminserviced startup only.  No return value.  Exits the process via
    ...
    def make_logger(self):
        # must be called after realize() and after adminservice does setuid()
    ...
    def make_http_servers(self, adminserviced):
        from adminservice.http import make_http_servers
        return make_http_servers(self, adminserviced)
    ...
    def reopenlogs(self):
        self.logger.info('adminserviced logreopen')
...
class ClientOptions(Options):
    ...
    def __init__(self):
        ...
        self.configroot = Dummy()
        self.configroot.rhadmin = Dummy()
        self.configroot.rhadmin.interactive = None
        self.configroot.rhadmin.prompt = 'redhawk'
        self.configroot.rhadmin.serverurl = None
        self.configroot.rhadmin.username = None
        self.configroot.rhadmin.password = None
        self.configroot.rhadmin.history_file = None

        from adminservice.rhadmin import DefaultControllerPlugin
        default_factory = ('default', DefaultControllerPlugin, {})
        # we always add the default factory. If you want to a rhadmin
        # without the default plugin, please write your own rhadmin.
        self.plugin_factories = [default_factory]

        self.add("interactive", "rhadmin.interactive", "i",
                 "interactive", flag=1, default=0)
        self.add("prompt", "rhadmin.prompt", default="adminservice")
        self.add("serverurl", "rhadmin.serverurl", "s:", "serverurl=",
                 url, default="http://localhost:9001")
        self.add("username", "rhadmin.username", "u:", "username=")
        self.add("password", "rhadmin.password", "p:", "password=")
        self.add("history", "rhadmin.history_file", "r:", "history_file=")
    ...
    def read_config(self, fp):
        section = self.configroot.rhadmin
        if not hasattr(fp, 'read'):
        ...
        parser = UnhosedConfigParser()
        parser.expansions = self.environ_expansions
        parser.mysection = 'rhadmin'
        parser.readfp(fp)
        sections = parser.sections()
        if not 'rhadmin' in sections:
            raise ValueError('.ini file does not include rhadmin section')
    ...
    def getServerProxy(self):
        ...
        return xmlrpclib.ServerProxy(
            ...
            'http://127.0.0.1',
            transport = xmlrpc.AdminServiceTransport(self.username,
                                                   self.password,
                                                   self.serverurl)
            )
...
class ProcessConfig(Config):
    ...
    def make_dispatchers(self, proc):
        ...
        dispatchers = {}
        from adminservice.dispatchers import POutputDispatcher
        from adminservice.dispatchers import PInputDispatcher
...
class EventListenerConfig(ProcessConfig):
    ...
    def make_dispatchers(self, proc):
        ...
        stdout_fd,stderr_fd,stdin_fd = p['stdout'],p['stderr'],p['stdin']
        dispatchers = {}
        from adminservice.dispatchers import PEventListenerDispatcher
        from adminservice.dispatchers import PInputDispatcher
        from adminservice.dispatchers import POutputDispatcher
        from adminservice import events
        if stdout_fd is not None:
...
class FastCGIProcessConfig(ProcessConfig):
    ...
    def make_process(self, group=None):
        ...
        from adminservice.process import FastCGISubprocess
    ...
    def make_dispatchers(self, proc):
        ...
        #file descriptor 0, so adminserviced cannot use stdin
...
class ProcessGroupConfig(Config):
    ...
    def make_group(self):
        from adminservice.process import ProcessGroup
...
class EventListenerPoolConfig(Config):
    ...
    def make_group(self):
        from adminservice.process import EventListenerPool
...
class FastCGIGroupConfig(ProcessGroupConfig):
    ...
    def make_group(self):
        from adminservice.process import FastCGIProcessGroup
...
class NoPermission(ProcessException):
    """ Indicates that the file cannot be executed because the adminservice
